/*
  I2Cx.cpp - I2C library
  Modified - Nov 13, 2012, Al Thomason
		- Added functions to support devices which do not use Register parameters(e.g. LCDs)
...
uint8_t I2C::write(uint8_t address, uint8_t registerAddress, uint8_t data, uint8_t data2)
{
  returnStatus = 0;
  returnStatus = start(); 
  if(returnStatus){return(returnStatus);}
...
uint8_t I2C::write(uint8_t address, uint8_t registerAddress, uint8_t data, uint8_t data2, uint8_t data3)
{
  returnStatus = 0;
  returnStatus = start(); 
  if(returnStatus){return(returnStatus);}
...
uint8_t I2C::write(uint8_t address, char *data)
{
  uint8_t bufferLength = strlen(data);
  returnStatus = 0;
  returnStatus = write(address, (uint8_t*)data, bufferLength);
  return(returnStatus);
}
